...
NSString * const ZZAppLanguageDidChangeNotification = @"ZZAppLanguageDidChangeNotification";
...
+ (void)setCusLanguage:(NSString *)language {
    ...
    if (!language) {
        ...
        if ([udfLanguageCode containsString:@"zh"]) {
            language = LaungeZH;
        }else{
            language = LaungeEN;
        }
    ...
    if ([self respondsToSelector:@selector(mj_localizedStringForKey:)]) {
        [self mj_localizedStringForKey:language];
    }
